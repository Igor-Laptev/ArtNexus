...
const { Post , User, Comment, Like , Category, Gallery, Art} = require('../../db/models');

//  ВЫВОД ВСЕХ ПОСТОВ

router.get('/', async (req, res) => {
   try {
     const posts = await Post.findAll({include:[{model: User}, {model: Comment}, {model: Like}, {model: Category}, {model: Gallery, include: [{model: Art}]}]});
...
router.get('/:id', async (req, res) => {
  try {
      const {id} = req.params;
      const post = await Post.findByPk(id, {include:[{model: User}, {model: Comment}, {model: Like}, {model: Category}, {model: Gallery, include: [{model: Art}]}]});
...
router.post('/', async (req, res) => {
  try {
    ...
    const newPost = await Post.create({user_id, description, title, category_id});
    const post = await Post.findByPk(newPost.id, {include:[{model: User}, {model: Comment}, {model: Like}, {model: Category}, {model: Gallery, include: [{model: Art}]}]});
